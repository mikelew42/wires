...
var Prime = Number.extend({
	...
	init: function(){
		...
		this.log.group("creating prime:", this.number);
		// this.log(this);
				prime.createPowers();
	...
	createPowers: function(){
		this.log.group("creating powers for prime:", this.number);
		this.powers = [];
		...
		do {
			...
			if (!this.numbers[result]){
				this.log("adding primePower " + power.number + "(" + power.factorization + ")" + " to numbers");
				this.numbers[result] = power;
			} else {
				this.log.info(power.number + " already exists");
			}
		...
		} while (result < limit && this.number !== 1);
		this.log.end();
	},
...
var PrimePower = Number.extend({
	...
	init: function(){
		...
		this.factors = {};
		this.factors[this.number] = this;
		this.log("creating PrimePower:", this.number, this.factorization);
...
var test1 = function(j){
	...
	for (var i = 0; i < j+1; i++){
		...
		if (rand2 === 1)
			rand2 = 2;
		test.assert(rand1*rand2 === multiply(rand1, rand2));
	}
...
test("numbers", function(){
	...
	// test.assert(multiply(2, 828) === 2*828);
	// debugger;
	// multiply(5, 16);

	// test.assert(multiply(2, 10) === 2*10);
	// for (var i = 2; i < 100; i++){
	// 	// console.log(i, (i+3)*2);
	// 	// console.log(multiply(i, (i+3)*2));
	// 	test.assert(multiply(i, (i+3)*2) === i * (i + 3)*2);
	// }
		...
		view({
			...
			content: function(){
				...
				this.$el.change(function(){
					// console.log(this);
					v.input = parseInt(v.$el.constructor(this).val());
					t.becomeCaptor();
					test1(v.input);
					t.restoreCaptor();
				})
