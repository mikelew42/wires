...
var Route = module.exports = Mod3.extend({
	...
	parsePath: function(){
		var bridgeParts;
		if (this.path && this.path.indexOf("/") > -1){
			bridgeParts = shared.cleanPathParts(this.path.split("/"));
			this.part = bridgeParts.pop();
	...
	bridge: function(bridgeParts){
		...
		// bridge between the route that created this, and this
		for (var i = 0; i < bridgeParts.length; i++){
	...
	makeFullPath: function(){
		var pathParts = [this.part];
		var parent = this.parent;

		while(parent){
			pathParts.unshift(parent.part);
